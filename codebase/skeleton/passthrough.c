...
static int fd_has_odirect(int fd)
{
	int flags = fcntl(fd, F_GETFL);
	if (flags == -1)
		return 0;
	return (flags & O_DIRECT) != 0;
}

static size_t get_fd_alignment(int fd)
{
	struct stat st;
	long pagesz = sysconf(_SC_PAGESIZE);
	if (fstat(fd, &st) == 0 && st.st_blksize > 0) {
		return (size_t)(st.st_blksize > pagesz ? st.st_blksize : pagesz);
	}
	return (size_t)pagesz;
}
		...
		if ((size % align) == 0 && (offset % align) == 0) {
			...
			if (((uintptr_t)buf % align) != 0) {
				...
				if (posix_memalign(&aligned_buf, align, size) == 0) {
					res = pread(fd, aligned_buf, size, offset);
					if (res != -1) {
						memcpy(buf, aligned_buf, res);
					} else {
						res = -errno;
					}
					free(aligned_buf);
		     ...
		     off_t offset, struct fuse_file_info *fi)
{
	...
	if (fd_has_odirect(fd)) {
		...
		if ((size % align) == 0 && (offset % align) == 0) {
			...
			res = pwrite(fd2, buf, size, offset);
			if (res == -1) res = -errno;
			close(fd2);
			if (fi == NULL) close(fd);
			return res;
