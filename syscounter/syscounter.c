 ...
 * Receives a pointer to the BPF skeleton (struct syscounter_bpf *skel), an array of counter_key structs to store the keys (struct counter_key keys[MAX_PIDS * MAX_OPS]) and an array of struct counter_val to store the values (struct counter_val values[MAX_PIDS * MAX_OPS]) as arguments
 * Returns the number of entries retrieved from the map
 */
int get_values_from_counter_map (struct syscounter_bpf *skel, struct counter_key keys[MAX_PIDS * MAX_OPS], struct counter_val values[MAX_PIDS * MAX_OPS]) {
	...
	cur_key = next_key;
	while (1) {
		struct counter_val counter_value;
 ...
 * Receives an array of counter_key structs (struct counter_key keys[MAX_PIDS * MAX_OPS]), an array of struct counter_val values (struct counter_val values[MAX_PIDS * MAX_OPS]), the number of entries in the arrays (int num_entries), an array of strings with the unique pid values (char pid_list[MAX_PIDS][30]), the number of unique pid values (int pid_count), an array of integers with the unique op values (int op_list[MAX_OPS]) and the number of unique op values (int op_count) as arguments
 * Prints the results in a table format with the pids as rows and the ops as columns, and the total for each op at the end
 */
void print_results(struct counter_key keys[MAX_PIDS * MAX_OPS], struct counter_val values[MAX_PIDS * MAX_OPS], int num_entries, char pid_list[MAX_PIDS][30], int pid_count, int op_list[MAX_OPS], int op_count) {
	struct counter_val total_op_list[MAX_OPS] = {0};

	printf("%-30s", "PID\\OP");
	for (int i = 0; i < op_count; i++) {
        char header[64];
        sprintf(header, "%s(C/B)", resolve_syscall(op_list[i]));
		printf("%-24s", header);
    }
	printf("\n");

	int total_width = 30 + op_count * 24;
	...
	for (int j = 0; j < pid_count; j++) {
		...
		for (int i = 0; i < op_count; i++) {
			struct counter_val val = {0, 0};
			for (int k = 0; k < num_entries; k++) {
				sprintf(comm, "%s-%d", keys[k].command, keys[k].pid);
				if ((strcmp(comm,pid_list[j])==0) && keys[k].op == op_list[i]) {
					val = values[k];
					total_op_list[i].calls += values[k].calls;
					total_op_list[i].bytes += values[k].bytes;
				}
			}
            char out[64];
            sprintf(out, "%llu/%llu", val.calls, val.bytes);
			printf("%-24s", out);
		}
		printf("\n");
	}
	...
	for (int i = 0; i < op_count; i++) {
        char out[64];
        sprintf(out, "%llu/%llu", total_op_list[i].calls, total_op_list[i].bytes);
		printf("%-24s", out);
	}
	printf("\n");
}
...
int main(int argc, char *argv[])
{
	...
	struct counter_val values[MAX_PIDS * MAX_OPS];
