 ...
 */
int update_counter(u32 pid, int op, u64 bytes) {

    struct counter_val init_val = { .calls = 1, .bytes = bytes };
    ...
    struct counter_val *val = bpf_map_lookup_elem(&counter, &key);
    if (val) {
        __sync_fetch_and_add(&val->calls, 1);
        __sync_fetch_and_add(&val->bytes, bytes);
        return 0;
    }

    // If not found, insert with init_val
...
int BPF_PROG(sys_enter_openat, struct pt_regs *regs, long syscall_id, int dfd, const char * filename, int flags, umode_t mode)
{
    ...
    return update_counter(pid, syscall_id, 0);
}
...
int BPF_PROG(sys_enter_write, struct pt_regs *regs, long syscall_id, u32 fd, char *buf, size_t count)
{
    ...
    return update_counter(pid, syscall_id, count);
}
